...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    formSubmit: function formSubmit(e) {
      e.preventDefault();
      var currentObj = this;
      console.log(this.userData.vin_number); //currentObj.$router.push({ name : 'inventory-view', params: { id: '123' } });

      axios__WEBPACK_IMPORTED_MODULE_9___default()({
        method: 'post',
        url: 'http://koboautos.com/twic-terminal/connect/api/create_inspections?vin_number=' + this.userData.value.vin_number + '&driver_name=' + this.userData.value.driver_name + '&company_name=' + this.userData.value.companyName + '&customer=Abdul sSalam'
      }).then(function (response) {
        console.log(response);
