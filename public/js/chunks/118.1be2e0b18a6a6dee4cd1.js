    ...
    customerformSubmit: function customerformSubmit(e) {
      ...
      axios__WEBPACK_IMPORTED_MODULE_10___default()({
        ...
        if (response.status == 200) {
          ...
          currentObj.customerswithcompany = response.data.newcustomer; //currentObj.customerswithcompany.push(response.data.newcustomer);

          currentObj.$root.$emit("bv::hide::modal", "modal-1"); //userData.customerName.value = response.data.value;

          userData.customerName = response.data.lastedit; //$('#modal-1').modal('hide');
