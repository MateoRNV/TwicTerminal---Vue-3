    ...
    customerformSubmit: function customerformSubmit(e) {
      ...
      axios__WEBPACK_IMPORTED_MODULE_10___default()({
        ...
        if (response.status == 200) {
          ...
          this.customerswithcompany.push(response.data.newcustomer); //this.$root.$emit("bv::close::modal", "modal-1");

          jquery__WEBPACK_IMPORTED_MODULE_1___default()('#modal-1').modal('hide'); //currentObj.$router.push({ path : '/inventory/view/'+response.data.key  });
          //location.reload();
        }
      })["catch"](function (response) {
        if (response.status == 422) {
